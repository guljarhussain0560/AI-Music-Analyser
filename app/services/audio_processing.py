...
from app.utils import downloader, extract_analytics, extract_lyrics, spleeter_wrapper, parallel_processor
    ...
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if "youtube.com" in source_url or "youtu.be" in source_url:
                # The robust downloader function will handle errors internally
                original_audio_path = downloader.download_from_youtube(source_url, output_location)
                original_title = os.path.splitext(os.path.basename(original_audio_path))[0]
            else:
                original_audio_path = downloader.download_from_spotify(source_url, output_location)
                original_title = os.path.splitext(os.path.basename(original_audio_path))[0]
            
            if not original_audio_path or not os.path.exists(original_audio_path):
                raise FileNotFoundError(f"Could not find or verify downloaded audio file in {output_location}")
            
            print(f"✅ Downloaded '{original_title}' to {original_audio_path}")
            
            # --- 2. Parallel Processing (Upload, Analyze, Split, Transcribe) ---
            s3_upload_url = None
            music_desc = None
            lyrics_text = ""
            ...
            print("🚀 Starting parallel processing: Uploading, Analyzing, Splitting, and Transcribing...")
            with ThreadPoolExecutor(max_workers=4) as executor:
                # Submit tasks to run in parallel
                s3_upload_future = executor.submit(s3_uploader.upload_file_to_s3, original_audio_path, f"songs/{user_id}/original_song/{uuid.uuid4()}{os.path.splitext(original_audio_path)[1]}")
                ...
                lyrics_future = executor.submit(extract_lyrics.transcribe_lyrics, original_audio_path)
                ...
                lyrics_text = lyrics_future.result()
            ...
            song_data_dto = schemas.SongCreateDTO(
                ...
                lyrics=lyrics_text, description=music_desc
            )
...
def process_audio_file_pipeline(db: Session, file_path: str, user_id: int):
    """
    Orchestrates the full processing pipeline for an audio file that has already
    been uploaded to the server.
    """
    start_time = time.time()
    try:
        # The API endpoint creates a temporary directory for the upload.
        # This pipeline will work within that directory's context.
        # We get the parent directory of the file_path to use as our temp_dir.
        temp_dir = os.path.dirname(file_path)
        print(f"✅ Processing started in temporary directory: {temp_dir}")

        # --- 1. Set File Path (Download step is skipped) ---
        original_audio_path = file_path
        # Extract a clean title from the uploaded filename
        original_title = os.path.splitext(os.path.basename(original_audio_path))[0]

        if not os.path.exists(original_audio_path):
            raise FileNotFoundError(f"Uploaded audio file not found at expected path: {original_audio_path}")

        print(f"✅ Processing uploaded file: '{original_title}' from {original_audio_path}")

        # --- 2. Parallel Processing (Upload, Analyze, Split, Transcribe) ---
        # This entire block is identical to your original pipeline.
        s3_upload_url = None
        music_desc = None
        lyrics_text = ""
        stems_output_dir = os.path.join(temp_dir, "stems")
        ...
        # The API endpoint will handle deleting the temporary directory.
        return {
            "songs_id": new_song.id,
            "splits_id": new_split_record.id,
        }
        ...
        print(f"❌ An error occurred during the file processing pipeline: {e}")
        if 'db' in locals() and db.is_active:
            db.rollback()
        return None